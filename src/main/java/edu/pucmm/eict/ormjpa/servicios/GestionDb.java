...
public class GestionDb<T> {
    ...
    public boolean eliminar(Object entidadId) throws PersistenceException{
        ...
        try {
            ...
            if (entidad != null) { // Check if the entity was found
                em.remove(entidad);
                em.getTransaction().commit();
                ok = true;
            } else {
                // If not found, rollback the transaction (though nothing happened)
                // and ensure 'ok' remains false.
                em.getTransaction().rollback();
                ok = false;
            }
        } catch (Exception e) { // Catch potential exceptions during commit/rollback
            if (em.getTransaction().isActive()) {
                em.getTransaction().rollback();
            }
            // Re-throw or handle the exception appropriately
            throw new PersistenceException("Error deleting entity with ID: " + entidadId, e);
        } finally {
            em.close();
        }
        return ok;
